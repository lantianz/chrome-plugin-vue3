...
- 普通构建输出目录固定为 `output/chrome-plugin-vue3`
- ZIP 文件名保留版本号，例如 `output/chrome-plugin-vue3_v1.0.1.zip`
...
3. ✅ 执行 Vite 构建
4. ✅ 将构建结果移动到 `output/chrome-plugin-vue3`
...
2. ✅ 执行 Vite 构建
3. ✅ 在待压缩目录内写入 `VERSION`
4. ✅ 创建 `output/chrome-plugin-vue3_v1.0.1.zip` 压缩包

**输出结果：**

```text
output/
└── chrome-plugin-vue3_v1.0.1.zip
```
...
例如当前正式版本是 `1.0.1`，则开发包在浏览器中会显示为：

- 名称：`A Chrome Extension [DEV]`
- 显示版本：`1.0.1-dev`
...
适合在本地反复加载已解压扩展。目录名固定为 `output/chrome-plugin-vue3`，避免因为目录名变化而被 Chrome 识别成新的插件目录。
...
- `output/chrome-plugin-vue3/VERSION`
- ZIP 解压后的 `chrome-plugin-vue3/VERSION`
...
2. **目录覆盖**：普通版本化构建会始终覆盖 `output/chrome-plugin-vue3`。如果你想保留历史产物，请额外复制备份，或直接使用 ZIP 文件归档。
...
**解决**：查看 `output/chrome-plugin-vue3/VERSION` 或 `manifest.json` 中的 `version` 字段。
...
- `scripts/extract-release-notes.js` - 从 `CHANGELOG.md` 提取指定版本发布说明
...
执行 vite build
    ↓
普通模式：移动构建结果到 output/chrome-plugin-vue3
    ↓
写入 VERSION 文件
    ↓
